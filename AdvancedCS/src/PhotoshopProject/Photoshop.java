    ...
    public void brighten(int amount) {
        ...
        for (int i = 0; i < pixels.length; i++) {
        	...
        	for (int j = 0; j < pixels[i].length; j++) {
        		...
        		pixels[i][j] = new Color(Math.min(pixels[i][j].getRed()+amount, 255), 
        				Math.min(pixels[i][j].getBlue()+amount, 255), Math.min(pixels[i][j].getGreen()+amount, 255));
        ...
        if (horizontally) {
        	...
        	for (int i = 0; i < pixels.length; i++) {
        		...
        		for (int j = 0; j < pixels[i].length/2; j++) {
        			...
        			Color c = pixels[i][j];
        			pixels[i][j] = pixels[pixels[i].length-i-1][j];
        			pixels[pixels[i].length-i-1][j] = c;
    ...
    public void negate() {
        ...
        for (int i = 0; i < pixels.length; i++) {
        	...
        	for (int j = 0; j < pixels[i].length; j++) {
        		...
        		pixels[i][j] = new Color(Math.max(255-pixels[i][j].getRed(), 0), 
        				Math.max(255-pixels[i][j].getBlue(), 0), Math.max(255-pixels[i][j].getGreen(), 0));
